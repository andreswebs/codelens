...
func TestRun_DebugTraceOnlyUnderDebug(t *testing.T) {
	...
	if code := run([]string{"codelens", "--debug", "bogus"}, strings.NewReader(""), &bytes.Buffer{}, &withStderr); code != 64 {
		t.Fatalf("--debug exit code = %d, want 64", code)
	}
	...
	if code := run([]string{"codelens", "bogus"}, strings.NewReader(""), &bytes.Buffer{}, &withoutStderr); code != 64 {
		t.Fatalf("no-debug exit code = %d, want 64", code)
	}
