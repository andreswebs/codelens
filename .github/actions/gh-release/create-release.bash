...
```sh
gh attestation verify codelens-linux-amd64-*.tar.gz --repo @REPO@
```
...
```sh
cosign verify-blob \
  --bundle codelens-*.spdx.json.sigstore.json \
  --certificate-identity-regexp "https://github.com/@REPO@/.github/workflows/release.yml@refs/tags/.*" \
  --certificate-oidc-issuer "https://token.actions.githubusercontent.com" \
  codelens-*.spdx.json
```
...
gh release create "${TAG}" \
    ...
    "${DIST_DIR}/codelens-${TAG}.spdx.json" \
    "${DIST_DIR}/codelens-${TAG}.spdx.json.sigstore.json"
