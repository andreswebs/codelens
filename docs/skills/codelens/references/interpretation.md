...
`.env.example`, `.github/workflows/*.yml`, `composer.json`, and `README`/docs files)
often outrank source. That is expected, not noise: unlike generated artifacts, these
are hand-maintained and must **not** be excluded. To keep the code hotspots legible,
