...
`0002-exit-code-taxonomy.md`) and the JSON-on-stdout,
...
- Every failure that can reach the user carries three things: a stable
  machine code (a `snake_case` string), an exit code from ADR 0002,
...
- Result and status are decoupled. When a command exits with a
  ...
  0002), the full result envelope is still written to stdout first;
