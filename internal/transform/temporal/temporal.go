...
// (exit 64) because the period is supplied directly via a flag.
var ErrInvalidPeriod = terr.New(
	"invalid_temporal_period", 64,
...
// is a data error (exit 65) because the offending value comes from the log.
var ErrInvalidDate = terr.New(
	"invalid_temporal_date", 65,
