...
func TestPrintLogCommand_DefaultOmitsAllUsesMailmap(t *testing.T) {
	...
	code := Run([]string{"print-log-command"}, Deps{In: strings.NewReader(""), Out: &stdout, Err: &stderr})
	if code != 0 {
...
func TestPrintLogCommand_AllFlag(t *testing.T) {
	var stdout, stderr bytes.Buffer
	code := Run([]string{"print-log-command", "--all"}, Deps{In: strings.NewReader(""), Out: &stdout, Err: &stderr})
	if code != 0 {
		t.Fatalf("exit code = %d, want 0; stderr:\n%s", code, stderr.String())
	}
	got := strings.TrimRight(stdout.String(), "\n")
...
func TestPrintLogCommand_AfterStillWorks(t *testing.T) {
	var stdout, stderr bytes.Buffer
	code := Run([]string{"print-log-command", "--after", "2024-01-01"}, Deps{In: strings.NewReader(""), Out: &stdout, Err: &stderr})
	if code != 0 {
		t.Fatalf("exit code = %d, want 0; stderr:\n%s", code, stderr.String())
	}
...
func TestPrintLogCommand_AllAndAfter(t *testing.T) {
	var stdout, stderr bytes.Buffer
	code := Run([]string{"print-log-command", "--all", "--after", "2025-01-01"}, Deps{In: strings.NewReader(""), Out: &stdout, Err: &stderr})
	if code != 0 {
		t.Fatalf("exit code = %d, want 0; stderr:\n%s", code, stderr.String())
	}
...
func TestPrintLogCommand_BadAfter(t *testing.T) {
	var stdout, stderr bytes.Buffer
	code := Run([]string{"print-log-command", "--after", "nope"}, Deps{In: strings.NewReader(""), Out: &stdout, Err: &stderr})
	if code != 64 {
	...
	if env.OK || env.Error.Code != "invalid_after_date" {
		t.Errorf("error envelope = %+v, want ok=false code=invalid_after_date", env)
	}
}
