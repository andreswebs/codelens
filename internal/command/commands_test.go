...
func TestCmd_Alias_Resolves(t *testing.T) {
	...
	// An unknown command/alias is a usage error (exit 64), not a silent no-op.
	var stdout, stderr bytes.Buffer
	if code := Run([]string{"authorz"}, Deps{In: strings.NewReader(sampleLog), Out: &stdout, Err: &stderr}); code != 64 {
		t.Fatalf("unknown alias exit code = %d, want 64; stderr:\n%s", code, stderr.String())
	}
}
...
func TestCmd_LogFile(t *testing.T) {
	...
	code := Run([]string{"authors", "--log", path}, Deps{In: strings.NewReader(""), Out: &stdout, Err: &stderr})
	if code != 0 {
		t.Fatalf("exit code = %d, want 0; stderr:\n%s", code, stderr.String())
	}
...
func TestCmd_MissingLog_EmptyStdin(t *testing.T) {
	var stdout, stderr bytes.Buffer
	code := Run([]string{"authors"}, Deps{In: strings.NewReader(""), Out: &stdout, Err: &stderr})
...
func TestCmd_Rows_Truncates(t *testing.T) {
	var stdout, stderr bytes.Buffer
	code := Run([]string{"authors", "--rows", "1"}, Deps{In: strings.NewReader(sampleLog), Out: &stdout, Err: &stderr})
	if code != 0 {
		t.Fatalf("exit code = %d, want 0; stderr:\n%s", code, stderr.String())
	}
...
func TestCmd_FlaggedAnalysis_EchoesEffectiveParams(t *testing.T) {
	...
	code := Run([]string{"coupling", "--min-coupling", "42"}, Deps{In: strings.NewReader(sampleLog), Out: &stdout, Err: &stderr})
	if code != 0 {
		t.Fatalf("exit code = %d, want 0; stderr:\n%s", code, stderr.String())
	}
