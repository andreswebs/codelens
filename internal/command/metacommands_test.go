...
// surfacing its --after flag and its error/exit-code contract, rather than
// a usage error.
func TestSchema_Command_PrintLogCommand(t *testing.T) {
	...
	if !equalStrings(got.ErrorCodes, []string{"invalid_after_date"}) {
		t.Errorf("error_codes = %v, want [invalid_after_date]", got.ErrorCodes)
	}
	if !equalInts(got.ExitCodes, []int{0, 64}) {
		t.Errorf("exit_codes = %v, want [0 64]", got.ExitCodes)
	}
}
...
func TestSchema_Command_Schema(t *testing.T) {
	...
	if !equalStrings(got.ErrorCodes, []string{"unknown_schema_command"}) {
		t.Errorf("error_codes = %v, want [unknown_schema_command]", got.ErrorCodes)
	}
	if !equalInts(got.ExitCodes, []int{0, 64}) {
		t.Errorf("exit_codes = %v, want [0 64]", got.ExitCodes)
	}
