...
func runAuthors(t *testing.T, log string, args ...string) authorsEnvelope {
	...
	code := Run(append([]string{"authors"}, args...), Deps{In: strings.NewReader(log), Out: &stdout, Err: &stderr})
	if code != 0 {
...
// error (exit 64) with a coded error envelope on stderr.
func TestE2E_Pipeline_BadGlob(t *testing.T) {
	const log = `--a1--2024-01-01--Alice--c1
10	0	src/app.go
`
	var stdout, stderr bytes.Buffer
	code := Run([]string{"authors", "--exclude", "a[b"}, Deps{In: strings.NewReader(log), Out: &stdout, Err: &stderr})
	if code != 64 {
...
// TestE2E_Pipeline_BadFile classifies an unreadable --group path as an I/O
// error (exit 74) with a coded error envelope on stderr, never a stack trace.
func TestE2E_Pipeline_BadFile(t *testing.T) {
	const log = `--a1--2024-01-01--Alice--c1
10	0	src/foo.go
`
	var stdout, stderr bytes.Buffer
	code := Run([]string{"authors", "--group", filepath.Join(t.TempDir(), "missing.txt")}, Deps{In: strings.NewReader(log), Out: &stdout, Err: &stderr})
	if code != 74 {
		t.Fatalf("exit code = %d, want 74; stderr:\n%s", code, stderr.String())
	}
	if stdout.Len() != 0 {
		t.Errorf("stdout should be empty on error, got:\n%s", stdout.String())
	}
	...
	if errEnv.OK || errEnv.Error.Code != "input_file_open_failed" {
		t.Errorf("error envelope = %+v, want ok=false code=input_file_open_failed", errEnv)
	}
}
