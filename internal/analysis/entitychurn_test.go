...
func TestEntityChurn_MissingMetrics(t *testing.T) {
	...
	// churn guard rejects it with missing_metrics (exit code 65).
	mods := []model.Modification{
	...
	if got := churn.ErrMissingMetrics.ExitCode(); got != 65 {
		t.Errorf("ErrMissingMetrics exit code = %d, want 65", got)
	}
}
