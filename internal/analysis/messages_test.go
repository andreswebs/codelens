...
func TestMessages_NoMessagesLog(t *testing.T) {
	...
	if got := ErrMissingMessages.ExitCode(); got != 65 {
		t.Errorf("ErrMissingMessages exit code = %d, want 65", got)
	}
}
...
func TestMessages_DescriptorRegistered(t *testing.T) {
	...
	if !reflect.DeepEqual(d.ExitCodes, []int{0, 64, 65, 70, 74}) {
		t.Errorf("ExitCodes = %v, want [0 64 65 70 74]", d.ExitCodes)
	}
}
