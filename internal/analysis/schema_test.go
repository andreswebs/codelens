...
func TestSchema_BuildsFromDescriptor(t *testing.T) {
	...
	if len(got.ExitCodes) != 5 || got.ExitCodes[4] != 74 {
		t.Errorf("ExitCodes = %v, want [0 64 65 70 74]", got.ExitCodes)
	}
}
...
func TestList_MergesAndSorts(t *testing.T) {
	analyses := []Descriptor{
		{Name: "revisions", Summary: "r", ExitCodes: []int{0, 64, 65, 70, 74}},
		{Name: "authors", Aliases: []string{"a"}, Summary: "au", ExitCodes: []int{0, 64, 65, 70, 74}},
	}
	extra := []CommandSummary{
		{Name: "schema", Summary: "sc", ExitCodes: []int{0, 64}},
	}
